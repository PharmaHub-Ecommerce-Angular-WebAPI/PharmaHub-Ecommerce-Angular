...
@Component({
  selector: 'app-profile-medicines',
  standalone: true,
  ...
  templateUrl: './profile-medicines.component.html',
  styleUrl: './profile-medicines.component.css',
})
export class ProfileMedicinesComponent implements OnInit {
  ...
  ngOnInit(): void {
    const medicinePackages$ = this.apiService.getMedicinePHARMACY();
    const maxPrice$ = this.apiService.getmaxpricePackages();
  
    medicinePackages$.subscribe(
      (medicinePackages: any[]) => {
        const formattedMedicine = medicinePackages.map((pkg) => ({
          id: pkg.id,
          name: pkg.name,
          imgUrl: pkg.imageUrl,
        ...
        this.allPackages = [...formattedMedicine];
